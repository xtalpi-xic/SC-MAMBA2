...
class DataCollator:
    ...
    pad_token_id_mod: Optional[int] = None
    ...
    def _sample_or_truncate_plus_pad(
        ...
        mod_types: torch.Tensor,
        max_length: int,
    ) -> Tuple[torch.LongTensor, torch.Tensor]:
        assert len(genes) == len(expressions) == len(mod_types)
        if len(genes) == max_length:
            return genes, expressions, mod_types
        if len(genes) > max_length:  # sample or truncate
            if self.sampling:
                return self._sample(genes, expressions, mod_types, max_length)
            else:
                return genes[:max_length], expressions[:max_length], mod_types[:max_length]
        else:  # pad
            return self._pad(genes, expressions, mod_types, max_length)
    ...
    def _sample(
        ...
        if self.keep_first_n_tokens == 0:
            ...
            return genes[indices], expressions[indices], mod_types[indices]
    ...
    def _pad(
        ...
        mod_types = torch.cat(
            [
                mod_types,
                torch.full(
                    (max_length - len(mod_types),),
                    self.pad_token_id_mod,
