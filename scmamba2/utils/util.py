...
def compute_perturbation_metrics(
    ...
    #     if i in zero_rows:
    #         continue
    #     x_ = true_mean_perturbed_by_condition[i]
    #     y_ = pred_mean_perturbed_by_condition[i]

    #     genes_filtered = np.array(genes)
        
    #     # Plot scatter plot
    #     plt.figure()
    #     plt.scatter(x_, y_, alpha=0.5, label=i)
        
    #     # Highlight genes in the condition
    #     for j, gene in enumerate(genes_filtered):
    #         if gene in c.split('+'):
    #             plt.scatter(x_[j], y_[j], color='red', label=gene)
        
    #     plt.title(f'Scatter Plot for {c}')
    #     plt.xlabel('Truth')
    ...
    #     # Save the plot
    #     plot_filename = os.path.join(save_dir, f'scatter_plot_{i}.png')
